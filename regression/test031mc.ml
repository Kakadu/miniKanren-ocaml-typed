module Ast_pattern0 = struct
  ...
  type ('matched_value, 'k, 'k_result) t =
      T of (context -> 'matched_value -> 'k -> 'k_result)
...
module Ast_pattern = struct
  ...
  let parse (T f) ?on_error x k =
    try f { matched = 0 } x k
    with Expected expected ->
      match on_error with
      | None -> failwiths "%s expected" expected
      | Some f -> f ()
  ...
  module Packed = struct
    ...
    let parse (T (t, f)) x = parse t x f
  end

  let __ = T (fun ctx  x k -> incr_matched ctx; k x)

  (*let __' = T (fun ctx x k -> incr_matched ctx; k { loc; txt = x })*)

  let drop = T (fun ctx _ k -> incr_matched ctx; k)
  ...
  *)
  ...
  let ( ^:: ) (T f0) (T f1) =
    T (fun ctx  x k ->
      ...
      | x0::x1 ->
        ...
        let k = f0 ctx  x0 k in
        let k = f1 ctx  x1 k in
        k
      | _ -> fail  "::")
  ;;
  ...
  let pair (T f1) (T f2) = T (fun ctx  (x1, x2) k ->
    let k = f1 ctx  x1 k in
    let k = f2 ctx  x2 k in
    k
  );;

  let ( ** ) = pair

  let triple (T f1) (T f2) (T f3) = T (fun ctx  (x1, x2, x3) k ->
    let k = f1 ctx  x1 k in
    let k = f2 ctx  x2 k in
    let k = f3 ctx  x3 k in
    k
  );;

  let alt (T f1) (T f2) = T (fun ctx  x k ->
  ...
  let map (T func) ~f = T (fun ctx  x k -> func ctx  x (f k))
  (*let map' (T func) ~f = T (fun ctx  x k -> func ctx  x (f  k))*)
  let map_result (T func) ~f = T (fun ctx  x k -> f (func ctx  x k))

  let ( >>| ) t f = map t ~f

  let map0 (T func) ~f = T (fun ctx  x k -> func ctx  x (           k  f     ))
  let map1 (T func) ~f = T (fun ctx  x k -> func ctx  x (fun a   -> k (f a  )))
  let map2 (T func) ~f = T (fun ctx  x k -> func ctx  x (fun a b -> k (f a b)))
  (*
  ...
  let many (T f) = T (fun ctx l k ->
    k (List.map (fun x -> f ctx  x (fun x -> x)) l))
  ;;
